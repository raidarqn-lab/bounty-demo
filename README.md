...
Screenshot review badges now derive from that page's confirmed player/score rows; edits restore pending status. The single supplied screenshot is still only page 1; no extra evidence is fabricated. Rejection requires a reason and records a decision timestamp with zero points in shared in-memory history. My contributions and the R4 private-profile preview display the same submission decisions. These views simulate member/R4 access only: the public demo has no authentication or privacy enforcement and never writes to Portal or sends messages. Production needs authenticated per-member history and R4 access controls.

Code convention: the bounty board shows the shared request code, such as VSDWED-20260923. A submission appends the submitting player's stable six-digit member identifier, such as VSDWED-20260923-000001 or -000002. These suffixes are example member IDs, not sequence counters or extracted-player identifiers; production assignment must use an immutable unique member mapping. The demo member is 000001. Multiple members can submit against the same bounty; decisions are independent.
